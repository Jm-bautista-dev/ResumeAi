...
@section('title', 'AI Career Score & ATS Analysis')
...
<div class="min-h-screen bg-slate-950 text-slate-100 py-12 px-6">
    <div class="max-w-6xl mx-auto space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-850 pb-8">
            <div>
                <a href="{{ route('resume.edit', $resume) }}" class="text-slate-400 hover:text-white transition text-sm flex items-center gap-1 mb-2">&larr; Back to Editor</a>
            ...
            <div class="flex gap-3">
                <form action="{{ route('resume.analyze', $resume) }}" method="POST">
                    @csrf
                ...
                <form action="{{ route('resume.apply-fixes', $resume) }}" method="POST">
                    @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="relative w-48 h-48 mx-auto flex items-center justify-center">
                        ...
                        <svg class="w-full h-full transform -rotate-90">
                            ...
                            <circle cx="96" cy="96" r="80" stroke="url(#blue-grad)" stroke-width="12" fill="transparent" 
                                    stroke-dasharray="502" stroke-dashoffset="{{ 502 - (502 * $analysis->score) / 100 }}"
                        ...
                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                            <span class="text-5xl font-black tracking-tight text-white">{{ $analysis->score }}</span>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-bold text-white">{{ $analysis->ats_score }}%</span>
                            </div>
                            <div class="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                                <div class="h-full bg-blue-500 rounded-full" style="width: {{ $analysis->ats_score }}%"></div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-bold text-white">{{ $analysis->grammar_score }}%</span>
                            </div>
                            <div class="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                                <div class="h-full bg-indigo-500 rounded-full" style="width: {{ $analysis->grammar_score }}%"></div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-bold text-white">{{ $analysis->content_score }}%</span>
                            </div>
                            <div class="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                                <div class="h-full bg-purple-500 rounded-full" style="width: {{ $analysis->content_score }}%"></div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="font-bold text-white">{{ $analysis->keyword_score }}%</span>
                            </div>
                            <div class="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                                <div class="h-full bg-rose-500 rounded-full" style="width: {{ $analysis->keyword_score }}%"></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="space-y-4">
                        @foreach($analysis->suggestions as $s)
                            <div class="p-4 bg-slate-950 border border-slate-900 rounded-2xl flex items-start gap-4 hover:border-slate-850 transition">
                                ...
                                <div class="flex-grow">
                                    <p class="text-sm font-light text-slate-300">{{ $s }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <!-- Missing Keywords -->
                    @if (is_array($analysis->missing_skills) && count($analysis->missing_skills) > 0)
                        <div class="bg-slate-900/40 border border-slate-850 p-6 rounded-3xl space-y-4">
                    ...
                    <!-- Suggested Roles -->
                    @if (is_array($analysis->suggested_roles) && count($analysis->suggested_roles) > 0)
                        <div class="bg-slate-900/40 border border-slate-850 p-6 rounded-3xl space-y-4">
                ...
                <div class="bg-slate-900/40 border border-slate-850 p-6 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        ...
                        <p class="text-lg font-bold text-white">{{ str_replace('-', ' ', ucfirst($analysis->recommended_template)) }}</p>
                    ...
                    <a href="{{ route('resume.preview-template', ['resume' => $resume, 'template' => $analysis->recommended_template]) }}"
